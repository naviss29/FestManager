...
| Variable | Valeur |
|---|---|
| `BACKEND_URL` | URL **https://** sslip.io du backend (ex: `https://<uuid-backend>.167.235.134.247.sslip.io`) |
| `NGINX_ENVSUBST_TEMPLATE_VARS` | `BACKEND_URL` |

> **Important :** utiliser `https://` et non `http://`. Coolify force la redirection HTTP→HTTPS via Traefik sur les URLs sslip.io. nginx reçoit sinon un 301 au lieu d'une réponse proxy, ce qui génère un 502 Bad Gateway.  
> Le `proxy_ssl_verify off` est déjà configuré dans `nginx.conf.template` pour éviter la vérification du certificat Let's Encrypt en communication interne.
...
#### ⚠️ Nommage des containers Coolify — les services applicatifs ont un nom instable

**Contexte :** dans Coolify, les containers Docker n'ont pas tous le même schéma de nommage.

| Type de service | Format du nom Docker | Stabilité |
|---|---|---|
| PostgreSQL (plugin Coolify) | `<UUID>` (ex: `ivb6cflil2a440bqgusq9btr`) | ✅ Stable — utilisable comme hostname pour `SPRING_DATASOURCE_URL` |
| Backend / Frontend (Dockerfile) | `<UUID>-<timestamp>` (ex: `ahs0z0n...-095805085911`) | ❌ Change à chaque redéploiement |

**Conséquence :** il est impossible d'utiliser le nom de container Docker du backend comme `BACKEND_URL` dans le frontend — il change à chaque push.

**Fix :** utiliser l'URL **sslip.io en `https://`** du service backend (Traefik maintient un routing stable par label, indépendamment du nom du container) :

```
BACKEND_URL=https://<uuid-backend>.167.235.134.247.sslip.io
```

Le `proxy_ssl_verify off` est déjà configuré dans `nginx.conf.template` pour la communication interne sans vérification de certificat.

#### ⚠️ nginx crash-loop au démarrage — `host not found in upstream`

**Symptôme :** le container frontend redémarre en boucle au démarrage avec l'erreur `host not found in upstream "<hostname>"`.

**Cause :** nginx résout les noms de hosts dans les directives `proxy_pass` **au démarrage**. Si le backend n'est pas encore prêt, la résolution DNS échoue et nginx ne démarre pas.

**Fix :** différer la résolution DNS à l'exécution via une variable et le resolver Docker interne :

```nginx
resolver 127.0.0.11 valid=30s ipv6=off;  # DNS Docker interne

location /api/ {
    set $backend "${BACKEND_URL}";  # variable = résolution par requête, pas au démarrage
    proxy_pass $backend;
}
```

Ce pattern est déjà en place dans `nginx.conf.template`.

#### ⚠️ nginx EMFILE — `accept4() failed (24: Too many open files)`

**Symptôme :** nginx logue `accept4() failed (24: Too many open files)` et commence à rejeter des connexions.

**Cause :** la limite soft de file descriptors dans un container Alpine est 1024 par défaut. Chaque connexion HTTP consomme 1 fd côté client + 1 côté upstream = saturation rapide.

**Fix :** ajouter `worker_rlimit_nofile 65535` dans la configuration globale nginx (fichier `nginx.conf`, avant le bloc `events`) :
...
Ce paramètre est déjà en place dans `frontend/nginx.conf`.

#### ⚠️ Coolify — health check de déploiement timeout pendant le démarrage backend

**Symptôme :** Coolify marque le déploiement comme "unhealthy" et effectue un rollback alors que le backend démarre correctement. Les logs montrent :
...
**Cause :** Coolify lance son propre health check de déploiement (10 tentatives × 5s = 50s) sur `/actuator/health`. Spring Boot retourne HTTP **503** pendant toute la phase de démarrage (avant que le statut soit `UP`), indépendamment du `--start-period` du `HEALTHCHECK` Dockerfile.

**Fix :** activer les liveness/readiness probes Spring Boot dans `application-docker.yml` :
...
Puis pointer le `HEALTHCHECK` Dockerfile sur `/actuator/health/liveness` (répond **200 dès que le contexte Spring est chargé**, bien avant la fin des migrations Flyway) :
...
| Endpoint | Disponible | Retourne 200 quand |
|---|---|---|
| `/actuator/health/liveness` | Dès le chargement du contexte Spring (~10-15s) | Contexte Spring initialisé |
| `/actuator/health/readiness` | Plus tard | DB connectée + migrations Flyway terminées |
| `/actuator/health` | Dès le start | `UP` = readiness complète (= retourne 503 pendant le démarrage) |

Ces deux fichiers sont déjà mis à jour dans le projet.
...
| T06-08 | Fix health check Coolify — liveness probe Spring Boot (`/actuator/health/liveness`) | ✅ Validé |
| T06-09 | Fix nginx 502 — `BACKEND_URL` en `https://` + `proxy_ssl_verify off` + resolver Docker | ✅ Validé |
| T06-10 | Fix nginx EMFILE — `worker_rlimit_nofile 65535` dans `nginx.conf` global | ✅ Validé |
