...
| T07-09 | Configuration Railway : ajouter variable `GOOGLE_CLIENT_ID` sur le service backend | ⏳ En attente |
...
> 1. Créer le VPS Hetzner CX22 (Ubuntu 24.04) et installer Coolify
> 2. Configurer les 2 environments (staging / production) avec leurs 3 services chacun
> 3. Récupérer les 4 webhook URLs Coolify et les ajouter comme secrets GitHub
> 4. Créer les GitHub Environments `staging` et `production` (avec approbation manuelle sur production)
> 5. Ajouter `GOOGLE_CLIENT_ID` dans les variables du service backend Coolify
