...
> L'infrastructure Coolify + CI/CD est en place. Le CI est maintenant vert (111 tests backend + 91 tests frontend).
>
> 1. Ajouter `GOOGLE_CLIENT_ID` dans les variables du service backend Coolify (staging + prod) pour activer le bouton Google en production
> 2. Activer la protection de branche `main` sur GitHub (CI requis avant merge)
> 3. Respecter la discipline de branches : **développer sur `develop`**, merge vers `main` uniquement via PR validée par CI
